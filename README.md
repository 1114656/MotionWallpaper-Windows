...
> **项目状态：** `v0.1.0-alpha.1` 公开 Alpha 测试版。当前使用稳定的 Windows App SDK 1.8 维护版本；便携包尚未签名，不建议用于生产环境，详见 [发布清单](docs/RELEASING.md)。
...
完整构建后可生成不包含用户壁纸与配置的 Alpha 便携包：

```powershell
.\scripts\package-alpha.ps1
```

脚本会验证包内文件，并在 `artifacts` 目录生成 ZIP 和对应的 SHA-256 校验文件。该包目前未签名，仅作为公开预发布测试版本提供。

## 数据目录

媒体库固定为 `MotionWallpaper.exe` 同目录下的 `Wallpapers` 文件夹，设置和运行状态位于 `Config`。升级或覆盖程序文件时必须保留这两个目录。
